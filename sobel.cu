...
__global__
void f_sobel(float3* dst, size_t dst_pitch, uchar3* src, size_t src_pitch, size_t width, size_t height, bool avg)
{
	...
	if (avg)
	{
		...
		float gradient = clamp(sqrt(avgx*avgx + avgy*avgy), 0.0f, 1.0f);
